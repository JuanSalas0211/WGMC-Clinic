  ...
  <body class="bg-white text-text font-sans">
    ...
    <script type="text/babel">
      ...
      const donationLinks = [
        ...
        { key: 'sponsor', title: 'Sponsor a Patient', href: 'sponsor-a-patient.html', internal: true },
        // Brick by Brick is temporarily disabled. To restore, remove `disabled: true`.
        { key: 'brick', title: 'Brick by Brick', href: 'https://willgrundymedicalclinic.org/brick-by-brick/', disabled: true }
      ];

      const activeDonationLinks = donationLinks.filter((link) => !link.disabled);

      // Internal pages carry the visitor's language and stay in the same tab;
      // PayPal and Brick by Brick are off-site, so they open in a new one.
      const donateHref = (link, lang) => (link.internal ? `${link.href}?lang=${lang}` : link.href);
      const donateTargetProps = (link) => (link.internal ? {} : { target: '_blank', rel: 'noreferrer' });
      ...
      function Header({ t, lang, onToggleLang }) {
        ...
        const donationMenuItems = activeDonationLinks.map((d) => ({ ...d, label: d.key === 'general' ? t.generalDonation : d.key === 'sponsor' ? t.sponsorPatient : t.brickByBrick }));
                    ...
                    <div className={`menu-pop w-56 rounded-brand bg-white shadow-card border border-slate-200 p-2 ${donateOpen ? 'open' : ''}`}>
                      {donationMenuItems.map((item) => (
                        <a
                          key={item.key}
                          className="block px-3 py-2.5 rounded-md hover:bg-muted focus-visible:outline-none focus-visible:ring-2 ring-brand-secondary"
                          href={donateHref(item, lang)}
                          {...donateTargetProps(item)}
      ...
      function SupportWGMC({ t, lang }) {
        const links = activeDonationLinks.map((link) => ({
          ...link,
                  ...
                  <div className="mt-4 flex flex-wrap gap-3" ref={donateRef}>
                    ...
                    <div className="relative">
                      ...
                      <div className={`absolute right-0 top-full mt-3 z-10 ${donateOpen ? '' : 'pointer-events-none'}`} style={{ pointerEvents: donateOpen ? 'auto' : 'none' }}>
                        <div className={`menu-pop w-56 rounded-brand bg-white shadow-card border border-slate-200 p-2 ${donateOpen ? 'open' : ''}`}>
                          {activeDonationLinks.map((link) => (
                            <a
                              key={link.key}
                              className="block px-3 py-2.5 rounded-md hover:bg-muted focus-visible:outline-none focus-visible:ring-2 ring-brand-secondary"
                              href={donateHref(link, lang)}
                              {...donateTargetProps(link)}
